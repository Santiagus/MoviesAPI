...
async def test_lifespan_raise_exception():
    ...
    # Mock the utils.get_database_url_from_alembic_config function to raise an exception
    with patch(
        "movies_service.app.utils.get_database_url_from_alembic_config"
    ) as mock_get_database_url:
        mock_get_database_url.side_effect = Exception("Database URL retrieval failed")

        # Use pytest.raises to assert that the exception is raised when calling lifespan
        with pytest.raises(Exception) as exc_info:
            async with lifespan(app):
                pass  # You can add test code here if necessary

        # Optionally, you can assert additional details about the raised exception
        assert str(exc_info.value) == "Database URL retrieval failed"
